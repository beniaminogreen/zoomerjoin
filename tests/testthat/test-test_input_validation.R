...
test_that("Validates n_bands is positive", {
        expect_error(jaccard_inner_join(a,b, n_bands =  -5), regexp = "n_bands")
})

test_that("Validates band_width is positive", {
       expect_error(jaccard_inner_join(a,b, band_width = -5), regexp = "band_width")
})

test_that("validates n_gram_width is positive", {
        expect_error(jaccard_inner_join(a,b, n_gram_width = -5), regexp = "n_gram_width")
})
...
test_that("Throws error when no shared columns", {
        ...
        expect_error(jaccard_inner_join(a2,b), regexp = "Can't Determine")
})
...
test_that("Match Col Must be in the dataset and of length one", {
        ...
        expect_error(jaccard_inner_join(a2,a2, by = c('ajaaj', "ahah")), regexp = "length 1")
        expect_error(jaccard_inner_join(a2,b, by = c('a'="b")), regexp = "by_a")
        expect_error(jaccard_inner_join(a,b, by = c('string'="b")), regexp = "by_b")
})
