...
- block_by:
  ...
  A named vector indicating which column to block on, such that rows
  that disagree on this field cannot be considered a match. Format
...
- threshold:
  ...
  considered a match (default is .95). The similarity is equal to 1 -
  the Jaccard distance between the two strings, so 1 implies the strings
  are identical, while a similarity of zero implies the strings are
...
- similarity_column:
  ...
  a column with this name giving the Jaccard similarity between the two
